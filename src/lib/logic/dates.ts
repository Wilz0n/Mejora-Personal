...
/**
 * Antigüedad del usuario en MESES CALENDARIO transcurridos desde `createdAt`
 * hasta `ref` (por defecto "ahora" en la timezone dada).
 *
 * El mes de registro cuenta como el mes 1 (antigüedad = 1). Se calcula por
 * diferencia de mes calendario (año*12 + mes), no por días exactos, para que
 * el "primer mes activo" sea 1 desde el día del registro.
 *
 * Ej.: registrado en ago 2026, en ago 2026 → 1; sep → 2; feb 2027 → 7.
 */
export function monthsSinceCreation(
  createdAt: Date,
  ref?: Date,
  timezone?: string,
): number {
  const now = ref ?? (timezone ? nowInTimezone(timezone) : new Date());
  const created = new Date(createdAt);
  const diff =
    (now.getFullYear() - created.getFullYear()) * 12 +
    (now.getMonth() - created.getMonth());
  // +1 para que el propio mes de registro cuente como el mes 1.
  return Math.max(1, diff + 1);
}

/**
 * Índice de mes calendario absoluto (año*12 + mes, 0-based) de una fecha.
 * Útil para comparar meses sin importar el día.
 */
 ...
 *
 * Si se pasa `createdAt`, NUNCA se generan claves de meses anteriores al mes
 * de creación de la cuenta (el historial arranca en el primer mes de uso).
 */
export function lastMonthsDayKeys(
  months: number,
  ref?: Date,
  timezone?: string,
  createdAt?: Date,
  ...
  const month = date.getMonth();
  const minIndex =
    createdAt !== undefined ? absoluteMonthIndex(new Date(createdAt)) : -Infinity;
  for (let offset = months - 1; offset >= 0; offset--) {
    const d = new Date(year, month - offset, 1);
    if (absoluteMonthIndex(d) < minIndex) continue; // antes del registro → se omite
 ...
 *
 * Si se pasa `createdAt`, se omiten los meses anteriores al mes de creación de
 * la cuenta, de modo que el progreso mes a mes arranca en el primer mes de uso.
 */
