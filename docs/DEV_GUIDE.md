...
- **Marcar gasto como pagado (doble clic/tap):** `src/components/finanzas/FixedExpenseItem.tsx` — client component. Detecta doble clic (desktop, `onDoubleClick`) y doble tap (móvil). Toggle optimista del campo `paidThisMonth` vía acción `toggleExpensePaid` en `src/app/actions/finance.ts`. Visual: fondo `bg-green-500/15` + borde verde + icono `check_circle` filled + line-through + badge `verified`. Tip informativo verde (fuera del panel, arriba de la sección Gastos Fijos) explica la interacción al usuario. Campo BD: `FixedExpense.paidThisMonth` (Boolean, default false).
...
- **Reinicio mensual automático (lazy):** `ensureMonthlyRollover(userId)` en `src/app/actions/finance.ts`, invocado al **renderizar `/finanzas`** (no hay cron; Vercel serverless). Al detectar un mes nuevo respecto a `FinancialSummary.lastFinanceResetMonth`: (1) archiva el cierre del mes anterior con `writeMonthlySnapshot` (helper extraído de `saveMonthlyFinance`), (2) resetea `monthlySavings = 0`, borra los `MicroExpense` y pone `paidThisMonth = false`, (3) fija `lastFinanceResetMonth = mes actual`. **Idempotente** y timezone-aware; **conserva** ingreso, gastos fijos y proyectos. No llama `revalidatePath` (corre en render). Campo BD nuevo: `FinancialSummary.lastFinanceResetMonth (String?)`. Aviso al usuario: bloque informativo bajo el subtítulo del header de `/finanzas`.
- **Historial Financiero (`/finanzas/historial`):** página server con 3 modos vía `?view=monthly|quarterly|semiannual` (patrón `ViewToggle` como Hábitos). Datos: `getFinanceHistory(userId)` en `data.ts` → entradas ascendentes. Agrupación: función pura `groupFinanceHistory(entries, period)` + `effectiveMonthlySavings` + tipos `FinanceHistoryPeriod`/`FinanceHistoryGroup`/`FinanceHistoryEntry` en `finance-logic.ts` (`FINANCE_HISTORY_GROUP_SIZE = {monthly:1, quarterly:3, semiannual:6}`; agrupa desde el mes más reciente hacia atrás). Botón de acceso: **"Ver Historial"** en `/finanzas/mes` (debajo de "Editar Finanza").
...
- **Archivado del 7º mes (reinicio de ciclo):** al alcanzar `ARCHIVE_THRESHOLD_MONTHS = 7` de antigüedad efectiva (`isArchiveDue`, calculada desde `getUserTenureStart` = `max(createdAt, dataResetAt)`):
  ...
  - Botón **"Ver Análisis / Backup"** (junto a Exportar) → ruta oculta `src/app/backup-analisis/page.tsx` (fuera del grupo `(app)`, sin sidebar). El usuario sube el JSON exportado; se parsea y se analiza con `analyzeBackup` (lógica pura en `src/lib/logic/backup-analysis.ts`) para mostrar el consolidado histórico: KPIs globales, progreso de hábitos mes a mes, distribución de pagos mensuales, ahorro histórico y gastos hormiga. Traducido al design system "Nocturne".
  - El export (`getUserExportData`) incluye `monthlyFinances[]` (histórico mensual: ingreso, ahorro, gastos fijos/hormiga, categorías, `savingsConfirmed`) y `microExpenses[]`, necesarios para que el análisis tenga datos reales. El CSV añade las secciones `# FINANZAS MENSUALES` y `# GASTOS HORMIGA`.
...
- **Data:** `getSavingsHistory` expone `savingsConfirmed` por mes y el `totalAccumulated` **descuenta** los meses con `savingsConfirmed === false` (cuentan como 0). `getMonthlyFinance` devuelve `savingsConfirmed`. `getMonthlyConfirmStates(userId)` alimenta la detección.
- **Componentes:**
  - `src/components/finanzas/SavingsConfirmationBanner.tsx` — **reemplaza al antiguo modal invasivo**. Banner sutil y cerrable (no bloquea), mensaje "¿Cómo te fue con tu ahorro de [Mes]? · Pactado: $XXX". Acciones: **Sí** (`updateMonthlySavings confirmed:true`), **Ajustar/Aportar** (abre `UpdateSavingsModal`), **No ahorré** (`confirmed:false`), **X descartar** (solo oculta en la sesión, no altera el registro → vuelve a aparecer mientras siga `null`). `useTransition` + `router.refresh()`.
  - `src/components/finanzas/UpdateSavingsModal.tsx` — modal ligero **solo de suma** ("Aportar más") + export `AdjustSavingsButton`. Envía `mode:"add"` y `confirmed:true`. Para reemplazar la cifra total se usa `SetSavingsButton` ("Editar ahorro"), evitando redundancia.
- **Montaje:** `src/app/(app)/finanzas/page.tsx` y `src/app/(app)/finanzas/mes/page.tsx` obtienen la timezone (`getUserTimezone`) y los estados (`getMonthlyConfirmStates`), calculan `pendingSavingsConfirmation(now)` y renderizan `SavingsConfirmationBanner` arriba. La tarjeta de Ahorro Mensual en `/finanzas` incluye `AdjustSavingsButton` ("Aportar más") junto a `SetSavingsButton` ("Editar ahorro").
...
- **QuickCSS (temas):** el tema se **persiste en la BD** (`User.quickCss`, aditivo/nullable → `npm run db:push` no borra datos) para sincronizarse a todos los dispositivos del usuario. Archivos: `updateQuickCssSchema` en `validators.ts`; Server Actions `setQuickCss` (guardar; vacío → `null`) y `clearQuickCss` (**Restablecer BORRA el registro**, no deja basura) en `actions/settings.ts`; `getUserProfile` devuelve `quickCss`. Inyección **server-side sin FOUC** con `src/components/comun/QuickCssStyle.tsx` (`<style>` en el HTML del servidor, montado en `(app)/layout.tsx`). El editor (`src/components/settings/QuickCSSEditor.tsx`, overlay Portal, responsive `100dvh`) usa **vista previa en vivo** con un `<style>` propio (`quickcss-live-preview` en `src/lib/quick-css.ts`) que NO toca el nodo de React. Plantilla + variables `--lt-*` en `src/lib/constants/default-css.ts`; hooks por página con `[data-page]` vía `PageScope.tsx`.
  - ⚠️ **Bug resuelto:** el enfoque previo (`localStorage` + `<script>`/manipulación de `<head>` en cliente) causaba `Cannot read properties of null (reading 'removeChild')` al navegar en producción, porque tocaba nodos que React controla. Ahora el tema se inyecta server-side y el preview usa un nodo separado propio.
...
Server Component. Muestra ingreso, ahorro, gastos fijos (con ícono, marcables como pagado vía `FixedExpenseItem`) y proyectos, todos editables (`FinanceModals`, `AddProjectButton`, `RemoveProjectButton` con modal de confirmación, `ContributeButton`). La tarjeta de Ahorro Mensual ofrece **"Editar ahorro"** (`SetSavingsButton`, reemplaza el total) y **"Aportar más"** (`AdjustSavingsButton`, suma); ambos escriben el mismo ahorro. Arriba de la sección de gastos fijos hay un tip informativo verde que explica la interacción de doble clic/tap. El botón `SaveFinanceButton` guarda el cierre del mes. Al cierre del mes (o inicio del siguiente) puede montar el **banner sutil** `SavingsConfirmationBanner` si `pendingSavingsConfirmation(now)` detecta un mes con `savingsConfirmed === null`.
...
- No hay `eval` en el código. El único `dangerouslySetInnerHTML` es el de `QuickCssStyle` (inyección server-side del tema QuickCSS leído de `User.quickCss`): es CSS del propio usuario dentro de un `<style>` (no ejecuta JS) y se acota por Zod (`updateQuickCssSchema`, máx. 200 KB).
